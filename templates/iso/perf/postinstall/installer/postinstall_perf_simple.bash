...
for counter in \$(seq 1 "$MAX_LOOPS"); do
    ...
    echo "---------------------------------------------------------------------"
    fio --loops="$FIO_LOOPS" --size="${FIO_SIZE}m" --output="$RESULT_DIR/.fiomark.txt" --filename="$RESULT_DIR/.fiomark.tmp" \
      ...
      --name=Seqwrite --bs="${FIO_SIZE}m" --iodepth=1 --numjobs=1 --rw=write 2> $RESULT_DIR/errors.log
    sleep "$SLEEP_BETWEEN"
    cat "$RESULT_DIR/.fiomark.txt"
    QUERY='def read_bw(name): [.jobs[] | select(.jobname==name+"read").read.bw] | add / 1024 | floor;
       ...
       job_summary("Seq")'
    read -d '\n' -ra V <<< "\$(jq "$QUERY" "$RESULT_DIR/.fiomark.txt")"
    echo ""
    echo "VEC: ${V[*]}"
    echo "Read: ${V[0]}MB/s, Write: ${V[2]}MB/s"
